return function()
	describe("Schedule Status Transitions", function()
		...
		it("Should handle aborted status", function()
			...
			time = 60
			schedule.update()
			assert(event:get_status() == "aborted", "Event should be aborted when condition fails with on_fail abort")
		end)
		...
		it("Should handle failed status", function()
			...
			time = 60
			schedule.update()
			assert(event:get_status() == "failed", "Event should be failed when condition fails with custom on_fail")
		end)
		...
		it("Should allow startable statuses to transition to active", function()
			...
			time = 60
			schedule.update()
			assert(event2:get_status() == "aborted" or event2:get_status() == "failed", "Event2 should be aborted or failed")
			assert(event3:get_status() == "failed" or event3:get_status() == "aborted", "Event3 should be failed or aborted")
		...
		it("Should handle all seven statuses", function()
			...
			assert(events.aborted:get_status() == "aborted" or events.aborted:get_status() == "failed", "Should have aborted or failed status")
			assert(events.failed:get_status() == "failed" or events.failed:get_status() == "aborted", "Should have failed or aborted status")
