...
---@param ordered_event_ids string[] Event ids in update order, see `state.get_ordered_event_ids`
...
function M.update_chained_events(ordered_event_ids, current_time, last_update_time, is_startable_status, update_event)
	...
	while continue_chain and passes < MAX_CHAIN_PASSES do
		...
		for _, event_id in ipairs(ordered_event_ids) do
			...
			if event_state and type(event_state.after) == "string" then
				local after_event_id = event_state.after
				local after_status = state.get_event_state(after_event_id)
				if after_status and M.is_chain_parent_ready(after_status, current_time) then
					if is_startable_status(event_state.status) or event_state.status == "paused" then
						if not event_state.start_time or event_state.start_time < after_status.end_time then
							event_state.start_time = M.get_chain_start_time(event_state, after_status, current_time)
